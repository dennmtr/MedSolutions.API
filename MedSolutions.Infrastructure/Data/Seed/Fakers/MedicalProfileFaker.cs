...
{
    public static Faker<MedicalProfile> CreateFaker(Guid userId, List<Enums.MedicalSpecialty> medicalSpecialtyIds, List<Enums.PatientPairType> patientPairTypeIds, Dictionary<Enums.MedicalSpecialty, List<Enums.AppointmentType>> appointmentTypes, string? locale = "en")
    {
        ...
        Faker<MedicalProfile> faker = new Faker<MedicalProfile>(locale)
            ...
            .RuleFor(p => p.SubscriptionStartDate, (f, p) => p.SubscriptionStartDate == DateTime.MinValue ? f.Date.Past(1) : p.SubscriptionStartDate)
            .RuleFor(p => p.SubscriptionEndDate, (f, p) => p.SubscriptionEndDate == DateTime.MinValue ? f.Date.Future(1) : p.SubscriptionEndDate)
            ...
            .FinishWith((f, p) => {
                ...
                p.Patients = PatientFaker
                    .CreateFaker(p.Id, appointmentTypes[p.MedicalSpecialtyId.Value], locale!)
                    .Generate(50);
